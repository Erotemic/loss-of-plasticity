...
class GenerateAndTest(object):
    ...
    def test_features(self, features):
        ...
        features_to_replace = {
            ...
            for n in self.meta.layer_names
        }
        ...
        num_features_to_replace = {0 for n in self.meta.layer_names}
    ...
    def gen_new_features(self, features_to_replace_input_indices, features_to_replace_output_indices, num_features_to_replace):
        ...
        """
        with torch.no_grad():
            for layer_name in self.meta.layer_names:
    ...
    def update_optim_params(self, features_to_replace,
        ...
        if self.opt_type == 'adam':
            for layer_name in self.layer_names:
                ...
                feat_idx = features_to_replace[layer_name]
                self.opt.state[layer.weight]['exp_avg'][feat_idx, :] = 0.0
                self.opt.state[layer.bias]['exp_avg'][feat_idx] = 0.0
                self.opt.state[layer.weight]['exp_avg_sq'][feat_idx, :] = 0.0
                self.opt.state[layer.bias]['exp_avg_sq'][feat_idx] = 0.0
                self.opt.state[layer.weight]['step'][feat_idx, :] = 0
                self.opt.state[layer.bias]['step'][feat_idx] = 0
                # output weights
                self.opt.state[next_layer.weight]['exp_avg'][:, feat_idx] = 0.0
                self.opt.state[next_layer.weight]['exp_avg_sq'][:,
                                                                feat_idx] = 0.0
                self.opt.state[next_layer.weight]['step'][:, feat_idx] = 0
